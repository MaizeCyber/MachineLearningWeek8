...
class DecisionTree():
    ...
    def show_accuracy(self):
        ...
        if self.tree_depth > 1:
            ...
            plt.savefig('results/DecisionTreeConfusionMatrix.png', bbox_inches='tight')
            plt.show()
            ...
            plt.ylabel('Feature')
            plt.tight_layout()
            plt.savefig('results/DecisionTreeFeatures.png', bbox_inches='tight')
            plt.show()
            ...
            graph.render(f"results/decision_tree_{self.tree_depth}_{self.impurity_metric}", format="png", view=False, cleanup=True)
